...
> Drop this at the repo root. Companion doc: `docs/architecture.html` — the **architecture** (hardware · app · protocol · state). That file carries no rationale and no history; this one carries no step-by-step detail. When a decision here changes something the spec asserts, update both.
...
| **Concept reconfirmed after 2026-07-20 rethink** | Alternatives surveyed against the 2026 landscape (MeshCore, Reticulum, TinyGS, sonde/APRS firmware). That survey was written up in the old master plan's §14 and **deliberately dropped** when the doc was rebuilt on 2026-07-25 (history does not belong in the spec); the record of it is the BUILDLOG entry for 2026-07-20. Shared field node stands: one node, multiple phones, everyone on one map. (The client later moved from PWA to native Android — see the superseding row above.) |
...
> **Screenshots of the built app** live in `docs/screens/` and are laid out in `docs/architecture.html` §03. **Recaptured 2026-08-01** against the mock node on an emulator, so the Link and Config screens now show BLE as a live transport. Still nothing captured against the real board.
...
| Reading the console on macOS | `idf.py monitor` exits with *"requires standard input to be attached to TTY"* from any non-interactive shell, and hand-rolled pyserial reads return nothing at all | Three separate causes, all silent: macOS has no `timeout` command; opening the port asserts **RTS, which holds EN low and keeps the chip in reset**; and DTR low straps IO0 into the ROM downloader, whose only output is one banner. What works: `esptool --before default_reset --after hard_reset run`, then open the port and **assert DTR and RTS together** before reading. `cat /dev/cu.usbmodem*` returns *nothing at all* — the USB-Serial-JTAG console only emits once the host CDC looks connected, and the `cu.` device deliberately does not assert modem control, which is the same property that makes it safe. Both lines high is the combination that neither resets the chip nor straps IO0. The `/dev` node also lingers for a moment across a reset, so reopen by *retrying the open until it succeeds* rather than by waiting for the path to reappear. |
| `key()` around `viewModel()` | Picking a node, or switching transport, changes nothing until the app is force-stopped — and then works, which makes it look like a caching problem | `key()` keys the *composition*; `ViewModelProvider` keys the *instance* by class name and returns the cached one without ever calling the factory. A long-lived ViewModel must be **re-aimed** (`retarget`), not rebuilt — per-key instances would leak, since a `ViewModelStore` has no public per-key removal. |
...
| One record per `send_bin` call | Backlog over BLE drops the client a few records in — the phone stays connected and subscribed but vanishes from the roster, so it looks like the *node* rebooted | A 32-byte record per notification is sixty notifications against a mbuf pool of about a dozen, and a chunk sized from a 517-byte MTU holds fifteen records. The session hands over **runs**; the transport chunks them. |
| A full notification pool read as a dead client | Working phones evicted mid-backfill, worse the faster the link | `send_bin` answers `ESP_ERR_TIMEOUT` for "nothing went out, ask again" — all-or-nothing, so the cursor never lies — and only a real refusal drops the client. Never send a burst from the NimBLE host task: it is the task that drains the pool, so waiting on it for mbufs cannot succeed. |
| A session-side drop the transport never hears | One client's chat arrives under another's callsign, and a disconnect evicts a stranger | Freeing the session slot is half the job: the transport still holds a peer that believes it is client N, and N goes to the next phone to arrive. `lg_tx_t.on_drop` closes the wire. Not called from `lg_session_leave()`, where the transport is already the one telling. |
| A stale `BluetoothGattCallback` | `send` returns false forever with nothing wrong on the wire, or the UI quotes an MTU this link never agreed to | The retry loop starts the next attempt without joining the old one, and Android delivers the old callback until its `BluetoothGatt` closes. Each attempt claims a token; callbacks that do not own it return. |
