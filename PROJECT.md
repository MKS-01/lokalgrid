...
| **Concept reconfirmed after 2026-07-20 rethink** | Alternatives surveyed against the 2026 landscape (MeshCore, Reticulum, TinyGS, sonde/APRS firmware) — see `docs/architecture.html` §14. Shared field node stands: one node, multiple phones, everyone on one map. (The client later moved from PWA to native Android — see the superseding row above.) |
...
> **Screenshots of the built app** live in `docs/screens/` and are laid out in `docs/architecture.html` §03. Captured 2026-07-25, Phase 02 against the mock — **due for recapture**: the Link and Config screens now report BLE as a live transport rather than as an unbuilt phase, and nothing here has been captured against the real board.
