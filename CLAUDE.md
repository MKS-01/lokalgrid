...
**Running on hardware** *(2026-07-26)*: both I²C buses inventoried at boot with every answering address named; the AXP2101 read before anything is written to it; **ALDO4 identified as the GNSS rail** and switched on; the **GNSS delivers real fixes** (UART1, rx 9 / tx 8, 9600) parsed from GGA/RMC/GSA into 32-byte records; `hello.mode` reports `gnss` or `synthetic` so a demo track can never pass as a position; LittleFS mounted; the OLED showing ssid, clients, BLE state, power source and GNSS state with no phone attached; **BLE advertising with a GATT service**; the SoftAP serving `proto 2` at `ws://192.168.4.1/ws`.
