...
- **`docs/lokalgrid-master-plan.html`** — the *reference spec*, in three parts: the hardware, the app, and the protocol where they meet, plus a state section listing what runs today and what is next. Only current, decided material — no history, no narrative.
...
**Phases 00–02 are done** apart from two items (Room for the track, and the two-client flood test), and **Phase 03 is substantially done**: the board in hand runs the firmware in this repo and serves the app.

**Running on hardware** *(2026-07-26)*: both I²C buses inventoried at boot with every answering address named; the AXP2101 read before anything is written to it; **ALDO4 identified as the GNSS rail** and switched on; the **GNSS delivers real fixes** (UART1, rx 9 / tx 8, 9600) parsed from GGA/RMC/GSA into 32-byte records; `hello.mode` reports `gnss` or `synthetic` so a demo track can never pass as a position; LittleFS mounted; the OLED showing an inverted header badged with clients-against-cap over ssid, wifi, ble (clients + MTU), gnss (satellites + HDOP, or the age of a lost fix) and power with uptime, all readable with no phone attached; **BLE advertising with a GATT service**; the SoftAP serving `proto 2` at `ws://192.168.4.1/ws`.

**One protocol, two transports.** `proto 2` lives once in `firmware/main/session.c` behind a transport interface; `net_ws.c` and `ble_gatt.c` are thin adapters. Do not add a second copy of the protocol for a new wire — a client is a client whichever wire it arrived on, and the roster names its transport. The BLE service is `6f6b616c-6772-6964-0000-000000000001` (`…0002` control, `…0003` records in the §4 chunk framing); NimBLE takes those bytes **little-endian**, and the service UUID must stay in the **scan response** or the advertisement exceeds 31 bytes and advertising silently dies.

**Hardware facts that contradict older notes:** PSRAM is **quad**, not octal (octal mode boot-loops). There are **two I²C buses** (i2c0 sda 17 / scl 18: OLED, BME280, magnetometer; i2c1 sda 42 / scl 41: PMU, RTC). This unit has **no QMI8658**. The BME280 **is** fitted but not yet read. The board runs from **USB, not a cell**, so there is no battery percentage and the power ladder has nothing to measure.

**Still unverified:** the app's BLE path against the node, and a USB-JTAG breakpoint. Both are next.
